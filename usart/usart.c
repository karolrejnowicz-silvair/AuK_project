...
#define USARTx_RX_GPIO_CLK_ENABLE()      __GPIOB_CLK_ENABLE() //a
#define USARTx_TX_GPIO_CLK_ENABLE()      __GPIOB_CLK_ENABLE() //a

#define USARTx_FORCE_RESET()             __USART1_FORCE_RESET()
#define USARTx_RELEASE_RESET()           __USART1_RELEASE_RESET()

/* Definition for USARTx Pins */
#define USARTx_TX_PIN                    GPIO_PIN_6 //a9
#define USARTx_TX_GPIO_PORT              GPIOB  
#define USARTx_TX_AF                     GPIO_AF7_USART1
#define USARTx_RX_PIN                    GPIO_PIN_7 //a10
...
bool USART_PutChar(char c)
{
	...
	//wrzuc char c do transmit buffer i uruchom przerwanie
...
void USARTx_IRQHandler(void){
	...
	if (__HAL_USART_GET_FLAG(&UartHandle, USART_FLAG_RXNE)) {
		...
		if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_RXNE)) {
			...
			data2 = USARTx->DR;
			if(data2){
				RingBuffer_PutChar(&USART_RingBuffer_Rx, data2);
			}
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
	    // the TXE interrupt is enabled
			...
			if (RingBuffer_GetChar(&USART_RingBuffer_Tx, &data)) {
				USARTx->DR = data;
			} else {
